...
async function searchUsers(query, currentUserId) {
  const q = sanitizeInput(query).replace(/[%_,.()]/g, '');
  if (!q) return [];
  const { data, error } = await supabase
    .from('users')
...
async function lookupByPhone(phone, currentUserId) {
  ...
  const { data, error } = await supabase
    ...
    .neq('id', currentUserId)
    .eq('is_verified', true)
    .ilike('phone_number', `%${clean.slice(-10)}%`)
    .limit(5);

  if (error) throw error;

  const match = (data || []).find(u => formatPhone(u.phone_number) === clean || formatPhone(u.phone_number).endsWith(clean));
  return match || null;
}
