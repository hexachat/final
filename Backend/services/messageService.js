...
async function deleteForEveryone(messageId, userId) {
  ...
  return { message: 'Deleted for everyone', messageId, conversation_id: msg.conversation_id };
}
...
async function addReaction(messageId, userId, reaction) {
  ...
  if (error) throw error;
  return { ...data, conversation_id: msg?.conversation_id };
}
...
async function removeReaction(messageId, userId) {
  ...
  await supabase.from('message_reactions').delete()
    .eq('message_id', messageId).eq('user_id', userId);
  return { message: 'Reaction removed', conversation_id: msg?.conversation_id };
}
