...
router.delete('/:messageId/everyone', authenticate, async (req, res) => {
  try {
    ...
    const io = req.app.get('io');
    if (io && result.conversation_id) {
      io.to(`conversation:${result.conversation_id}`).emit('message_deleted', result);
    }
    res.json({ success: true, ...result });
  } catch (err) {
    res.status(400).json({ success: false, message: err.message });
...
router.post('/:messageId/reaction', authenticate, [body('reaction').notEmpty()], validate, async (req, res) => {
  try {
    ...
    const io = req.app.get('io');
    if (io && reaction.conversation_id) {
      io.to(`conversation:${reaction.conversation_id}`).emit('message_reaction', { message_id: req.params.messageId, reaction });
    }
...
router.delete('/:messageId/reaction', authenticate, async (req, res) => {
  try {
    ...
    const io = req.app.get('io');
    if (io && result.conversation_id) {
      io.to(`conversation:${result.conversation_id}`).emit('message_reaction_removed', { message_id: req.params.messageId, user_id: req.userId });
    }
    res.json({ success: true, ...result });
  } catch (err) {
    res.status(400).json({ success: false, message: err.message });
